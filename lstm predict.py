...
导入数据
"""
flight_data=pd.read_excel('s13.xls')
...
plt.plot(flight_data['指标值'])
plt.show()
"""
数据预处理
"""
flight_data.columns#显示数据集中 列的数据类型
all_data = flight_data['指标值'].values.astype(float)#将passengers列的数据类型改为float
#划分测试集和训练集
test_data_size = 12
...
同样,第二个序列从第二个数据开始,到第13个数据结束,第14个数据是第二个序列的标签,依此类推。
"""
...
plt.grid(True)
plt.autoscale(axis='x', tight=True)
plt.plot(flight_data['指标值'][-train_window:])
plt.plot(x, actual_predictions)
plt.show()
arima=[269360.69584625, 254574.0567838 , 245768.23811671 ,238059.39659495,
 231752.77923814 ,226539.42768509 ,222236.85597584 ,218685.04106619,
 215753.10075498,213332.83725507 ,211334.95564015 ,209685.74200578]
summodel=[]
actual_predictions_sum=sum(list(actual_predictions))
for i in range(12):
    x=(arima[i]/sum(arima))*actual_predictions_sum
    summodel.append(float(x))
print(summodel)

plt.title('s1线客流预测组合模型')
plt.ylabel('乘客数')
plt.xlabel('天数')
plt.grid(True)
plt.autoscale(axis='x', tight=True)
plt.plot(flight_data['指标值'])
plt.plot([ 96,97 , 98 , 99 ,100, 101, 102 ,103, 104, 105, 106, 107],summodel)
plt.show()