...
data_csv=pd.read_excel('s13.xls')
 
plt.plot(data_csv)
plt.show()
data_csv=data_csv['指标值'].values.astype('float32')
'''
...
dataset = scaler.fit_transform(data_csv.reshape(-1, 1))
#查看归一化之后的前5条数据和后5条数据
print(data_csv[:5])
print(data_csv[-5:])
#将数据集转换为tensor，因为PyTorch模型是使用tensor进行训练的，并将训练数据转换为输入序列和相应的标签
data_csv = torch.FloatTensor(data_csv).view(-1)
